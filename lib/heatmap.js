...
function renderHeatmap(container, days, options = {}) {
  ...
  // Anchor the window by its end: Sunday of today's week minus (weeks-1) full
  // weeks, so the last column is the current week and today's cell always
  // renders. (Rolling back from "today minus N days" dropped today's cell 6
  // days out of 7.)
  const start = addDays(o.today, -((o.weeks - 1) * 7 + o.today.getDay()));
  ...
  for (let w = 0; w < o.weeks; w++) {
    for (let r = 0; r < 7; r++) {
      ...
      if (key > todayKey) {
        ...
        cell.setAttribute("aria-label", `${date.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })} — future`);
