...
function wrappedYearStats(days, articles, settings, today = new Date()) {
  ...
  for (const k of Object.keys(days)) {
    if (k >= startKey && k <= todayKey) window[k] = days[k];
  }
  const windowArticles = articles.filter((article) => article && article.date >= startKey && article.date <= todayKey);
  const stats = computeStats(window, settings);
  const sum = summarizeReading(window, windowArticles, settings);
  const topics = topicDistribution(windowArticles, settings);
...
function drawWrapped(canvas, { days, articles, settings, today = new Date() }) {
  ...
  // Anchor the window by its end (Sunday of today's week minus (weeks-1)
  // full weeks) so today's cell always renders; days after today stay blank.
