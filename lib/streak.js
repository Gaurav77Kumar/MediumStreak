...
function dayIsActive(entry, settings) {
  const goal = Number.isFinite(entry && entry.goalMin) && entry.goalMin > 0
    ? entry.goalMin
    : (settings.dailyGoalMin || 10);
  return !!entry && (entry.frozen || (entry.minutes || 0) >= goal);
}
...
function computeStats(days, settings, today = new Date()) {
  ...
  const keys = Object.keys(days)
    .filter((k) => /^\d{4}-\d{2}-\d{2}$/.test(k) && !Number.isNaN(parseDateKey(k).getTime()) && localDateKey(parseDateKey(k)) === k)
  ...
  for (const k of Object.keys(days)) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(k) || Number.isNaN(parseDateKey(k).getTime()) || localDateKey(parseDateKey(k)) !== k) continue;
    const e = days[k];
